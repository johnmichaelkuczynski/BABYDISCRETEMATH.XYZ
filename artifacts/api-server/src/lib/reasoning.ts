...
function deterministicFeedback(
  ...
  return `Thanks for working through these discrete-math cases.${overall?.value ? ` You scored ${overall.value}.` : ""} Strong answers here come from reasoning about the case — pointing to the detail that decides it (is logic's OR inclusive? is the 'if' part of an implication even true here? does one input map to many outputs? are there more items than containers?), and resisting 'obvious' guesses like 'lots of examples prove it' or 'the pigeonhole principle is too obvious to matter'.`;
}
...
export async function generateFeedback(
  ...
  const system =
    instrument === "subject"
      ? "You are an instructor giving warm, specific feedback on a student's discrete-math reasoning check (realistic short cases about the course material — logic and truth values, connectives and quantifiers, proof and counterexamples, sets, relations, and functions, counting and the pigeonhole principle, graphs, modular arithmetic, recursion and induction). 2-4 sentences. Note their overall performance and offer one concrete way to reason better about cases. Use only the metrics provided; do not invent numbers. Plain prose, no markdown headings."
...
async function generateSubjectVariant(
  ...
  const system =
    "You are an assessment author writing ORIGINAL discrete-math questions for an intro course. " +
    "Every question must be a short, realistic CASE (a concrete scenario about discrete math — logic and truth values, connectives and quantifiers, proof and counterexamples, sets, relations, and functions, counting and the pigeonhole principle, graphs, modular arithmetic, or recursion and induction) that rewards REASONING about the case, never recall of a definition or a one-word fact. No heavy formal derivations — reward plain-language reasoning. " +
  ...
  const user =
    `Write ${count} new, distinct discrete-math case questions within the scope above.\n` +
    `For style only (do NOT copy these): ${JSON.stringify(examplePrompts)}.`;
  const out = await chatJson<{
