...
router.post("/practice/sessions/:sessionId/next", async (req, res): Promise<void> => {
  ...
  try {
    generated = await chatJson<{
      ...
      `You generate a single introductory discrete-math practice problem for a curious beginner. The problem MUST be on the topic "${topic.title}" and at difficulty "${difficultyLabel}" (${difficulty.toFixed(
        1,
      )}/5). Test general understanding of the SUBJECT of discrete math, not recall of any particular book, lecture, or course. The question MUST be fully self-contained and answerable by anyone who knows the discipline: do NOT reference "the lecture", "the text", "the course", "the example", "the case", or any named character or example a student would only recognize from a specific reading.\n\nSTRICT QUESTION RULES (no exceptions):\n- Every question must present a SPECIFIC, concrete discrete-math situation (e.g. a menu saying "soup or salad" and whether logic's inclusive OR forbids having both, why "if it rains, the ground is wet" is still true on a sunny day, whether "is a sibling of" is a function when one person has many siblings, why 13 people must include two who share a birth month, why a hash table with more keys than slots forces a collision, why a road map and a friendship network are the same dots-and-lines model, why 5 hours after 10 o'clock is 3 on a 12-hour clock, or why checking the first 100 numbers does not prove a rule for all numbers) and ask the student to APPLY the topic's idea to THAT situation — to judge, compare, explain, interpret, or decide. Explain ideas intuitively, in plain words — do NOT demand heavy symbolic manipulation or a memorized formula.\n- NEVER ask for a definition, a term, or what something is "called"; NEVER ask the student to recite an abstract formulation from any reading.\n- NEVER write a question whose answer is a single word, a single term, or a bare "yes"/"no". The answer must require reasoning that is HARD TO SHARE: the student must explain their thinking about the specific situation in 2-4 sentences.\n- The "correctAnswer" is the model answer (2-4 sentences) showing the operational reasoning a strong student would give for THIS situation. The "explanation" briefly says what earns full credit.\n\nRespond as strict JSON: {"prompt": string, "correctAnswer": string, "explanation": string}. Avoid these recent prompts: ${JSON.stringify(
    ...
    generated = {
      prompt: `Practice (${topic.title}): A friend sees a menu that says "soup or salad" and insists that, logically, this must forbid having both. Using the idea behind "${topic.title}", walk through how you'd help them see what actually follows step by step, and explain your reasoning in 2-4 sentences about that situation.`,
      correctAnswer:
    ...
    try {
      tutorTip = (
        await chatJson<{ tip: string }>(
          "You are a kind, concise discrete-math tutor. Given a problem, the correct answer, and the student's wrong attempt, give ONE focused next-step tip (2 sentences max). Respond as strict JSON: {\"tip\": string}.",
          JSON.stringify({
            prompt: problem.prompt,
            correctAnswer: problem.correctAnswer,
