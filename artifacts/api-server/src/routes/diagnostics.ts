    ...
    steps.push(
      ...
      await run(`Practice problem ${i + 1}: generate + grade + adapt`, async () => {
        ...
        const gen = await chatJson<{
          ...
          `You generate a single introductory discrete-math practice problem on "${topic.title}" at easy difficulty. The question must present a concrete discrete-math situation and ask the student to apply the topic's idea to it, with a model answer that reasons about the situation in 2-4 sentences (never a single word, term, or bare yes/no). Respond as strict JSON: {"prompt": string, "correctAnswer": string, "explanation": string}.`,
    ...
    steps.push(
      ...
      await run(
        ...
        async () => {
          ...
          // discrete math. Homework is scoped to its own topic's lecture, but
          ...
          const derived = await chatText(
            "You are a strong introductory discrete-math student answering a short-answer problem. " +
          ...
          const verdict = await chatJson<{
            ...
            "You are an academic quality-control reviewer for an introductory discrete-math course. " +
...
router.post("/diagnostics/expand-lectures", async (req, res) => {
  ...
  const sys =
    `You are an introductory discrete-math lecturer producing the ${level.toUpperCase()} version of a lecture. ` +
