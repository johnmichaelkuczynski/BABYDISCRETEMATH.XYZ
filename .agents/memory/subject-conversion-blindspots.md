...
## Audience repositioning (children → adults) is scattered beyond the obvious "audience" section

When the mandate is about *who the course is for* (this course is for ADULTS — grad students / professors entering a new discipline — never "children"/"middle schoolers"/"kids"), the audience wording hides in several layers, and fixing only the dedicated "Designed For / Who it's for" section is not enough:

- **The top-line tagline on line 3–4 of both `README.md` and `replit.md`** (the bold one-sentence pitch under the H1) is *separate* from the audience bullet section and easily missed — it said "for curious students and adults alike". A code review caught this after the audience section was already fixed.
- **In-app marketing copy:** `qr-course/index.html` `<meta description>`/`og`/`twitter` (3 identical strings — use `edit` with `replace_all`) and the landing hero subtitle (`Landing.tsx`) both carried "students and grown-ups"/"curious students".
- **Seeded lecture text idioms:** `seed.ts` lecture bodies use "a principle a child can understand" as a *figure of speech* for simplicity — grep for `child` flags it. Reword (e.g. "simple enough to state in one sentence") and **bump `SEED_CONTENT_VERSION`** so the live DB reseeds.
- **"no math!" marketing lead:** the same pass usually pairs with a user demand to stop leading with "no prior math / no coding / no jargon". That phrase recurs across `youtube-description.md`, `README.md`, `replit.md`, `index.html` metas, `Landing.tsx`, and the `course.ts` unit overview. Replace with content-first framing ("built up from first principles in plain language"); keep the substance, drop the dismissive claim, and never make it the opening line of the YouTube description.

**How to apply:** sweep for audience idioms, not just the audience section — `rg -i "students and adults|curious students|grown-?up|young|middle school|\bchild(ren)?\b|\bkids?\b"` plus `rg -i "no prior|no math|no coding|no jargon"`. UI `children` props, `attached_assets/`, framer `staggerChildren`, and `child_process` imports are false positives — exclude them.
